        ...
        if count:
            ...
            while k + package_size + 2 < data_len:  # pacakge size is 242
                if data_bytes[k] == 0X16 and data_bytes[k + 1] == 0X00 and data_bytes[k + package_size] == 0X16 and data_bytes[k + package_size + 1] == 0X00:
                    time_receive_data = time.time()
                    if file_open_flag:
                        custom_file.write("\r\n" + str(round(time_receive_data * 1000)) + " ")
                    package_data = np.zeros(dataset_count * led_count)
                    for dataset in range(dataset_count):
                        for led in range(led_count):
                            ...
                            rate[led] = rate[led] + 1
                    if file_open_flag:
                        custom_file.write(' '.join([str(item) for item in package_data]))
                    k = k + package_size
...
class MainWidget(QtWidgets.QMainWindow):
    def action_save(self):
        ...
        if self.saveButton.text() == "SaveData":
            ...
            custom_file_name, _file_ok = QFileDialog.getSaveFileName(self,
                                                         "文件保存",
                                                         "./",
                                                         "Text Files (*.txt)")
